...
class BackupVault(CFTemplate):
    def __init__(
        ...
        super().__init__(
            ...
            account_ctx,
            aws_region,
            config_ref=vault.aim_ref_parts,
            stack_group=stack_group,
            stack_tags=stack_tags
        )
        self.set_aws_name('BackupVault')
        self.init_template('Backup Vault')
        self.aim_ctx.log_action_col("Init", "Backup", "Vault")
        ...
        if vault.notification_events:
            notification_aim_ref = self.aim_ctx.project['resource']['notificationgroups'][aws_region][vault.notification_group].aim_ref + '.arn'
        ...
        for plan in vault.plans.values():
            ...
            for selection in plan.selections:
                ...
                if selection.resources:
                    resource_arns = []
                    for aim_ref in selection.resources:
                        from aim.models.references import get_model_obj_from_ref
                        obj = get_model_obj_from_ref(aim_ref, self.aim_ctx.project)
