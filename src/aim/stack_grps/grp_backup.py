...
class BackupVaultsStackGroup(StackGroup):
    ...
    def log_init_status(self, name, description, is_enabled):
        ...
        self.aim_ctx.log_action_col('Init', 'Backup', name, description, enabled=is_enabled)
    ...
    def create_iam_role(self):
        ...
        role = aim.models.iam.Role(role_name, self.config)
        role.apply_config(role_dict)

        iam_role_ref = self.config.aim_ref_parts + '.' + role_name
    ...
    def init(self):
        ...
        for backup_vault in self.config.values():
            ...
            backup_vault_template = aim.cftemplates.BackupVault(
                self.aim_ctx,
                self.account_ctx,
                self.region,
                self, # stack_group
