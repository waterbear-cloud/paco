...
class SecretsManagerStackGroup(StackGroup):
    def __init__(self, aim_ctx, account_ctx, env_ctx, config, stack_tags):
        super().__init__(
            aim_ctx,
            account_ctx,
            env_ctx.netenv_id,
    ...
    def log_init_status(self, name, description, is_enabled):
        ...
        self.aim_ctx.log_action_col('Init', 'Secrets', name, description, enabled=is_enabled)
    ...
    def init(self):
        ...
        secrets_template = aim.cftemplates.SecretsManager(
            self.aim_ctx,
            self.account_ctx,
            self.region,
            self, # stack_group
